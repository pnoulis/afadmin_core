...
* Developing Task Runner
idle state -> run a job -> is service connected -> pending | running

running -> runJobs -> flush -> run first async task -> runjobs returns

synchronous
flush -> new jobs to run
are there jobs to run -> yes | no
yes -> is state running

jobs -> []
run -> [1] -> pending
run -> [1, 2] -> pending
connected -> run jobs
flush jobs -> new jobs array
